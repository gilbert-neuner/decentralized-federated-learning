...
for (scheme, ) in product(["G", "AG", "A,G", "G,A", "S", "AS"]): 
    ...
    df_replicate, trust_replicate, beta_true = replicate_algorithm(algorithm_params = {"scheme": scheme, "clip": False, "threshold": threshold})
...
df = (
    df_out
        ...
        .groupby(["scheme"], as_index = False)[["F1", "Y_rel_norm"]]
        .mean()
)