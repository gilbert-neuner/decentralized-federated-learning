...
for scheme, shape, include in product(["G", "A", "(AC)", "(GC)"], ["band", "bridged ring"], [-0.5, 0, 0.5]):
    K = 20
    if shape == "band":
    ...
    grad_plot.set_title(f"Scheme = {scheme}, shape = {shape}, include = {include}, rel_norm = {np.mean(rel_norm_exp[all_friendly_ids]):.3f}, F1 = {np.mean(F1_exp[all_friendly_ids]):.3f}")
    ...
    model_plot.set_title(f"Scheme = {scheme}, shape = {shape}, include = {include}, rel_norm = {np.mean(rel_norm_exp[all_friendly_ids]):.3f}, F1 = {np.mean(F1_exp[all_friendly_ids]):.3f}")
    plt.plot([0, 1, 2, 3, 4, 5, 6], rel_norm_values, marker="o", color="red", linestyle="-")
    model_plot.legend(
        bbox_to_anchor=(1.05, 1),   # x=1.05, y=1 relative to axes
        loc='upper left',           # anchor point of the legend box
