...
class Communication_Network:
    ...
    def __init__(self, adjacency_matrix, beta0, X, Y, a, b, threshold):
        self.comm_graph = []
        self.K = np.shape(adjacency_matrix)[0]
        for k in range(self.K):
            neighbors = set(np.where(adjacency_matrix[k, :] == 1)[0])
            self.comm_graph.append(Client(client_id = k, neighbors = neighbors, X = X[k], Y = Y[k], beta_curr = beta0, a = a, b = b, threshold = threshold))
    ...
    def run_experiment(self, n_iter, scheme, threshold = 10**-0.5, max_step_size = 1):
        if scheme == "threshold":
            for iteration in range(n_iter):
                for k in range(self.K):
                    self.comm_graph[k].select_step_size(scheme = scheme, curr_iter = iteration, max_step_size = max_step_size)
        else:
            for iteration in range(n_iter):
        ...
        #         for k in range(self.K):
        #             self.comm_graph[k].select_step_size(scheme = scheme, curr_iter = iteration, max_step_size = max_step_size)
        # elif scheme == "models_gradients_threshold": 
        #     for iteration in range(n_iter):
        #         self.broadcast_models()
        #         self.compute_and_broadcast_gradients()
        #         for k in range(self.K):
